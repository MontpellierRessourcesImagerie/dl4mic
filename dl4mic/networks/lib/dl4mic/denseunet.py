...
from keras.layers import Input, Conv2D, MaxPooling2D, Dropout, concatenate, UpSampling2D, BatchNormalization, Activation
...
def create_patches(Training_source, Training_target, patch_width, patch_height):
  ...
  """
    ...
    for file in os.listdir(Training_source):
        ...
        all_patches_img = np.concatenate((all_patches_img,
                                          patches_img.reshape(patches_img.shape[0] * patches_img.shape[1], patch_width,
                                                              patch_height)), axis=0)
        all_patches_mask = np.concatenate((all_patches_mask,
                                           patches_mask.reshape(patches_mask.shape[0] * patches_mask.shape[1],
                                                                patch_width, patch_height)), axis=0)
    ...
    for i in range(number_of_patches):
        ...
        # if the mask contains at least 5% of its total number pixels as mask, then go ahead and save the images
        ...
        if pixel_threshold_array[int(round(len(pixel_threshold_array) * 0.95))] > 0:
            ...
            io.imsave(Patch_rejected + '/patch_' + str(i) + '_image.tif',
                      img_as_ubyte(normalizeMinMax(all_patches_img[i])))
            io.imsave(Patch_rejected + '/patch_' + str(i) + '_mask.tif',
                      convert2Mask(normalizeMinMax(all_patches_mask[i]), 0))
...
# My Version
def denseUnet(pretrained_weights=None, input_size=(256,256,1), pooling_steps=4, learning_rate=1e-4, verbose=True,
              class_weights=np.ones(2)):
    def denseBlock(input_tensor, filters):
        ...
        x2 = concatenate([x, input_tensor], axis=-1)
        x2 = BatchNormalization(axis=-1, epsilon=1.1e-5)(x2)
        x2 = Activation('relu')(x2)
        x2 = Conv2D(filters, 3, activation='relu', padding='same', kernel_initializer='he_normal')(x2)

        x3 = concatenate([x2, x, input_tensor], axis=-1)
        x3 = BatchNormalization(axis=-1, epsilon=1.1e-5)(x3)
        x3 = Activation('relu')(x3)
        x3 = Conv2D(filters, 3, activation='relu', padding='same', kernel_initializer='he_normal')(x3)
    ...
    # Downsampling
    for p in range(0, pooling_steps):
        if p == 0:
    ...
    #Bottom
    bottom = Conv2D(channels * pow(2, p), 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool)
    bottom = Conv2D(channels * pow(2, p), 3, activation='relu', padding='same', kernel_initializer='he_normal')(bottom)
    bottom = Dropout(0.5)(bottom)

    # Upsampling
    for p in range(pooling_steps - 1, -1, -1):
        if p == pooling_steps - 1:
            up = UpSampling2D(size=(2, 2))(bottom)
        else:
